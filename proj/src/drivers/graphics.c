            ...
            for (int32_t j = 0; j < sprite->height; j++, fy += fym)
                for (int32_t is = 0; is < scale; is++)
                    for (int32_t js = 0; js < scale; js++){
                        ...
                        Color color = SpriteGetPixel(sprite, fx, fy);
                        uint32_t color32 = (color.red << 16) | (color.green << 8) | color.blue;
                        vg_draw_pixel(x + i * scale + is, y + j * scale + js, color32);
