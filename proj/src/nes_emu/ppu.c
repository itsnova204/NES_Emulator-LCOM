...
bool ppu_clock()
{
	...
	if (bgPixel == 0 && fg_pixel == 0)//both transparent
	{
		pixel = 0x00;
		palette = 0x00;
	}
	else if (bgPixel == 0 && fg_pixel > 0)//background transparent, foreground not
	{
		pixel = fg_pixel;
		palette = fg_palette;
	}
	else if (bgPixel > 0 && fg_pixel == 0)//foreground transparent, background not
	{
		pixel = bgPixel;
		palette = bgPalette;
	}
